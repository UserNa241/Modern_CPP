...
int main() {
	...
	try {
		unsafe_credit(alice, 10);
	} catch (const std::exception &e) {
		std::cout << "  caught: " << e.what() << "\n";
	}
	std::cout << "  try_lock: " << (alice.mtx.try_lock() ? "GOT IT\n" : "FAILED (lock leaked)\n");
	...
	try {
		guarded_credit(alice, 10);
	} catch (const std::exception &e) {
		std::cout << "  caught: " << e.what() << "\n";
	}
	std::cout << "  try_lock: " << (alice.mtx.try_lock() ? "GOT IT (dtor ran)\n" : "FAILED\n");
	alice.mtx.unlock();
	...
	std::cout << "\n=== 5. Alphabet trick: always lock lower address first ===\n"; {
		std::jthread t1([&] {
			lock_ordered(alice.mtx, bob.mtx);
		});
		std::jthread t2([&] {
			lock_ordered(bob.mtx, alice.mtx);
		});
	}

	std::cout << "\n=== 6. scoped_lock both directions: no deadlock ===\n"; {
		std::jthread t1([&] {
			transfer_scoped(alice, bob, 7);
		});
		std::jthread t2([&] {
			transfer_scoped(bob, alice, 3);
		});
	}
